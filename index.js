...
let headPos = Object.assign({}, initalState.pos);
let body = structuredClone(initalState.body);
let isWaiting = true;
...
function moveSnake() {
    console.log("moving");
    let prevPart;
    let prevPos = Object.assign({}, headPos);
    headPos = {
        x: headPos.x + direction.x,
        y: headPos.y + direction.y
    };
    for (let part of body) {
        prevPart = Object.assign({}, part);
        part.x = prevPos.x;
        part.y = prevPos.y;
        prevPos = Object.assign({}, prevPart);
    }
    if (body.find(part => part.x == headPos.x && part.y == headPos.y)) {
        frutasComidas = 0;
        isWaiting = true;
        headPos = Object.assign({}, initalState.pos);
